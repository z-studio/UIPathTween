...
namespace Cysharp.Threading.Tasks.Editor {
    public class UniTaskTrackerViewItem : TreeViewItem {
        private static Regex s_RemoveHref = new("<a href.+>(.+)</a>", RegexOptions.Compiled);
    ...
    public class UniTaskTrackerTreeView : TreeView {
        private const string k_SortedColumnIndexStateKey = "UniTaskTrackerTreeView_sortedColumnIndex";
        ...
        private UniTaskTrackerTreeView(TreeViewState state, MultiColumnHeader header) : base(state, header) {
            rowHeight = 20;
            ...
            header.sortedColumnIndex = SessionState.GetInt(k_SortedColumnIndexStateKey, 1);
        }
        ...
        private void Header_sortingChanged(MultiColumnHeader multiColumnHeader) {
            ...
            switch (index) {
                case 0:
                    orderedEnumerable = ascending ? items.OrderBy(item => item.TaskType)
                        : items.OrderByDescending(item => item.TaskType);

                    break;
                case 1:
                    orderedEnumerable = ascending ? items.OrderBy(item => double.Parse(item.Elapsed))
                        : items.OrderByDescending(item => double.Parse(item.Elapsed));

                    break;
                case 2:
                    orderedEnumerable = ascending ? items.OrderBy(item => item.Status)
                        : items.OrderByDescending(item => item.Elapsed);

                    break;
                case 3:
                    orderedEnumerable = ascending ? items.OrderBy(item => item.Position)
        ...
        protected override TreeViewItem BuildRoot() {
            ...
            TaskTracker.ForEachActiveTask((trackingId, awaiterType, status, created, stackTrace) => {
                    children.Add(
                        new UniTaskTrackerViewItem(trackingId) {
