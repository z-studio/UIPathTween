    ...
    public static partial class UniTaskExtensions {
        ...
        public static UniTask<T> AsUniTask<T>(this Task<T> task, bool useCurrentSynchronizationContext = true) {
            ...
            task.ContinueWith(
                ...
                promise,
                useCurrentSynchronizationContext ? TaskScheduler.FromCurrentSynchronizationContext()
        ...
        private sealed class AttachExternalCancellationSource : IUniTaskSource {
            private static readonly Action<object> s_CancellationCallbackDelegate = CancellationCallback;
            ...
            public AttachExternalCancellationSource(UniTask task, CancellationToken cancellationToken) {
                ...
                m_TokenRegistration =
                    cancellationToken.RegisterWithoutCaptureExecutionContext(s_CancellationCallbackDelegate, this);
        ...
        private sealed class AttachExternalCancellationSource<T> : IUniTaskSource<T> {
            ...
            private async UniTaskVoid RunTask(UniTask<T> task) {
                try {
                    m_Core.TrySetResult(await task);
                } catch (Exception ex) {
                    m_Core.TrySetException(ex);
                } finally {
                    m_TokenRegistration.Dispose();
        ...
        public static async UniTask Timeout(
            ...
            try {
                (winArgIndex, taskResultIsCanceled, _) = await UniTask.WhenAny(
        ...
        static async UniTaskVoid ForgetCoreWithCatch(
            UniTask task,
            Action<Exception> exceptionHandler,
        ...
        private static async UniTaskVoid ForgetCoreWithCatch<T>(
            UniTask<T> task,
            Action<Exception> exceptionHandler,
            bool handleExceptionOnMainThread
        ) {
        ...
        public static async UniTask<TR> ContinueWith<T, TR>(
            this UniTask<T> task,
        ...
        private sealed class ToCoroutineEnumerator : IEnumerator {
            ...
            private Action<Exception> m_ExceptionHandler = null;
        ...
        private sealed class ToCoroutineEnumerator<T> : IEnumerator {
            private bool m_Completed;
            private Action<T> m_ResultHandler = null;
            private Action<Exception> m_ExceptionHandler = null;
            private bool m_IsStarted = false;
            ...
            private async UniTaskVoid RunTask(UniTask<T> task) {
                try {
                    ...
                    m_Current = value; // boxed if T is struct...

                    if (m_ResultHandler != null) {
                        m_ResultHandler(value);
                    }
                } catch (Exception ex) {
                    if (m_ExceptionHandler != null) {
                        m_ExceptionHandler(ex);
