...
namespace Cysharp.Threading.Tasks.Internal {
    ...
    internal sealed class ArrayPool<T> {
        ...
        private const int k_DefaultMaxNumberOfArraysPerBucket = 50;

        private static readonly T[] s_EmptyArray = new T[0];

        public static readonly ArrayPool<T> Shared = new();

        private readonly MinimumQueue<T[]>[] m_Buckets;
        private readonly SpinLock[] m_Locks;

        private ArrayPool() {
            // see: GetQueueIndex
            m_Buckets = new MinimumQueue<T[]>[18];
            m_Locks = new SpinLock[18];

            for (var i = 0; i < m_Buckets.Length; i++) {
                m_Buckets[i] = new MinimumQueue<T[]>(4);
        ...
        public T[] Rent(int minimumLength) {
            ...
            if (index != -1) {
                var q = m_Buckets[index];
                var lockTaken = false;

                try {
                    m_Locks[index].Enter(ref lockTaken);
        ...
        public void Return(T[] array, bool clearArray = false) {
            ...
            if (index != -1) {
                ...
                try {
                    ...
                    if (q.Count > k_DefaultMaxNumberOfArraysPerBucket) {
                        return;
                    }
