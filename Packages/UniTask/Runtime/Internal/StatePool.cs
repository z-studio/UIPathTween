...
namespace Cysharp.Threading.Tasks.Internal {
    ...
    internal static class StatePool<T1> {
        private static readonly ConcurrentQueue<StateTuple<T1>> s_Queue = new();

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static StateTuple<T1> Create(T1 item1) {
            if (s_Queue.TryDequeue(out var value)) {
                value.Item1 = item1;
                return value;
            }
    ...
    internal static class StatePool<T1, T2> {
        private static readonly ConcurrentQueue<StateTuple<T1, T2>> s_Queue = new();
